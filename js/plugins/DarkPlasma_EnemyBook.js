...
(() => {
  ...
  class Window_EnemyBookStatus extends Window_Base {
    ...
    refresh() {
      ...
      this.resetTextColor();
      //console.log(enemy)
      var enemyLv = enemy.meta["enemyLv"] ? Number(enemy.meta.enemyLv) : 0
      var race = enemy.meta["race"] ? enemy.meta.race : "unknown"
      var text = ` <${race}:LV ${enemyLv}>`
      this.drawText(enemy.name + text, 30, 30);
    ...
    drawPage() {
      ...
      if(race != "unknown"){
        ...
        var raceText = `\\c[16][種族特性:${race}]\\c[0]` + $dataStates[raceId].meta.raceDesc
        raceText = raceText.replace("弱点属性","\\c[6]弱点属性\\c[0]")
        raceText = raceText.replace("耐性属性","\\c[29]耐性属性\\c[0]")
        raceText = raceText.replace("BS耐性","\\c[29]BS耐性\\c[0]")
     ...
     */
    descriptionY() {
      return this.itemPadding() + this.lineHeight() * 10;
    }
