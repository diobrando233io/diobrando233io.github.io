...
if($gameActors._data[1]._equips[EqCloth]._itemId >= 5){
	...
	if(EqNum == 67 && $gameActors.actor(1).isStateAffected(94)) EqNum = 68
    if(EqNum == 67 && $gameActors.actor(1).isStateAffected(95)) EqNum = 69
...
var RopeFlag = 0;  //Expand6 b v h a
...
if(HairFlag >= 1){//头发换色
	...
	if(CLOTHTAG == "Change" && CUTINALTFLAG >= 1) spriteExpand5.setColorTone($gameActors.actor(1).hairToneb);
	else spriteExpand5.setColorTone($gameActors.actor(1).hairTone);
...
if(CUTINOPTIONFLAG >= 1 && UNDERFLAG >= 1){//タイツオフ、下着オンかつ未返信
  ...
  if($gameActors.actor(2).toneArray[k]) spriteOption.setColorTone($gameActors.actor(2).toneArray[k]);
...
if(CUTINTITESFLAG >= 1 && [300,301,307,309,310].includes($gameActors._data[1]._equips[EqLeg]._itemId) && ChangeFlag == false){//タイツフラグonかつ足装備ID300かつ未変身
  ...
  if($gameActors.actor(2).toneArray[LEGEQNUM]) spriteTites.setColorTone($gameActors.actor(2).toneArray[LEGEQNUM]);
...
if(EqNum >= 5 && CUTINCLOTHFLAG >= 1){//衣装フラグオンかつ5以上
  ...
  if($gameActors.actor(2).toneArray[EqNum]) spriteCloth.setColorTone($gameActors.actor(2).toneArray[EqNum]);
...
if(RopeFlag >= 1 && $gameActors._data[1]._equips[6]._itemId >= 5 && $dataArmors[$gameActors._data[1]._equips[6]._itemId].meta.FileNumRope){//特殊物品
	var FileNameRope = $dataArmors[$gameActors._data[1]._equips[6]._itemId].meta.FileNumRope;
	ExpandName6 = "actor01_cutin_" + CUTINBASENUM + "_option_" + FileNameRope;
	var bitmap = ImageManager.loadPicture(ExpandName6);
	var spriteExpand6 = new Sprite(bitmap);
	SceneManager._scene._spriteset.addChild(spriteExpand6); spriteExpand6.x = Cutin1X; spriteExpand6.y = Cutin1Y;
	SceneManager._scene.Cutin1Expand6 = spriteExpand6;
}
...
//動かす処理、スプライトに置き換える場合上部の処理変更
    if(SceneManager._scene.Cutin1Dif2){
	  ...
	  if(SceneManager._scene.Cutin1Expand6){
        Torigoya.Tween.create(SceneManager._scene.Cutin1Expand6)
      .to({x: CutinAnimeX,y: CutinAnimeY},AnimeWait, Torigoya.Tween.Easing.easeOutSine)
      .to({x: Cutin1X,y: Cutin1Y},AnimeWait, Torigoya.Tween.Easing.easeOutSine).start()
      }
    }
...
function EraceCutin1Expand() {
  ...
  if(SceneManager._scene.Cutin1Expand6) {
    SceneManager._scene._spriteset.removeChild(SceneManager._scene.Cutin1Expand6);
    SceneManager._scene.Cutin1Expand6 = null;
  }
};
