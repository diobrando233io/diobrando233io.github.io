    ...
    if (command === 'CallStand' || command === 'CallStandForce') {
	  ...
	  if(StandPoseID == 11){//土下座
		...
		if($gameSwitches.value(131)){
			...
			$gameScreen.showPicture(35,'submit/2',0,Stand1X,Stand1Y,scale,scale,255,0);
		}else{
			submitCloth = 14;
			$gameScreen.showPicture(35,'submit/1',0,Stand1X,Stand1Y,scale,scale,255,0);
		}
		FileName = 'submit/' + submitCloth;
		$gameScreen.showPicture(53,FileName,0,Stand1X,Stand1Y,scale,scale,255,0);
		this.wait(10);
		return;
	  }
	  ...
	  //状态
		...
		if($gameActors.actor(1).isStateAffected(321)){
			if($gameActors.actor(1).mark) Mark1 = $gameActors.actor(1).mark;
			else Mark1 = "6";
			if([6,7].includes(StandPoseID)) Mark1 = "0";}
		if($gameActors.actor(1).isStateAffected(322)) Mark2 = "10";
		//涂鸦
      ...
      var stand_base = stand_bigin + 1; //136、201、229
	  //贴身
	  var stand_scar = stand_bigin + 2;
	  var stand_paint = stand_bigin + 3;
	  var stand_mark1 = stand_bigin + 4;	
	  var stand_mark2 = stand_bigin + 5;	  
      ...
      var stand_under = stand_bigin + 16;  //299
      var stand_leg = stand_bigin + 17;  //299
	  var effect_splash = stand_bigin + 18;
      var stand_cloth = stand_bigin + 19; //乳环处理到此为止 299
	  var stand_rope = stand_bigin + 20;
      var stand_neck = stand_bigin + 21; 
	  var stand_byt = stand_bigin + 22; //1043
      var stand_semenbody = stand_bigin + 23;
      var stand_semenface = stand_bigin + 24;
      var stand_semenmouth = stand_bigin + 25; //尾巴处理到此为止
	  var effect_breath = stand_bigin + 36; //229
      var stand_diffront = stand_bigin + 66; //299
      ...
      if($gameActors._data[1]._equips[StandEqCloth]._itemId >= 5){
		...
		if(StandEqNum == 67 && $gameActors.actor(1).isStateAffected(94)) StandEqNum = 68
        if(StandEqNum == 67 && $gameActors.actor(1).isStateAffected(95)) StandEqNum = 69
		...
		if(StandEqNum == 77 && $gameActors.actor(1).isStateAffected(95)) StandEqNum = 78
      ...
      if($gameActors._data[1]._equips[11] && $gameActors._data[1]._equips[11]._itemId >= 5){
         MouthPicFileNum = $dataArmors[$gameActors._data[1]._equips[11]._itemId].meta.FileNumMouth;
      }
	  ...
	  var RopePicFileNum = 0;
      if($gameActors._data[1]._equips[6]._itemId >= 5 && $dataArmors[$gameActors._data[1]._equips[6]._itemId].meta.FileNumRope){
         RopePicFileNum = $dataArmors[$gameActors._data[1]._equips[6]._itemId].meta.FileNumRope;
      ...
      if(eyeId >= 5){
         EarPicFileNum = $dataArmors[eyeId].meta.FileNumEar;
		 //处理尾巴
		 if(StandPoseID == 6){
			stand_ear += 12;
			stand_pierceL -= 1;
      ...
      if(StandAltFlag >= 1){
		  if($gameSwitches.value(98)) var BaseID = "0006";
			  ...
			  if(StandPoseID <= 2 && id >= 5){
				weaponIndex = $dataWeapons[id].meta.weaponIndex ? $dataWeapons[$gameActors._data[1]._equips[0]._itemId].meta.weaponIndex : 1;
				if($dataWeapons[id].meta.feat){
					if($gameActors.actor(1).isStateAffected(392) || window.weaponEffect) weaponIndex += 'b';
				}
			  }
		  }
      }else{
      ...
      if($gameScreen.picture(stand_cloth) && $gameScreen.picture(stand_cloth)._name == FileName && ClothUpdate == 0){
        ...
        if(ClothPicFileNum != 0){//取得した装備タグの衣装ファイル名が0(全裸)以外の場合      
		  ...
		  var P = new Game_Picture();
		  P.show(FileName,0,Stand1X,Stand1Y,scale,scale,EqClothOpacity,0);
	  ...
	  //束缚
	  FileName = BasePoseFileName + "_option_" + RopePicFileNum
      if($gameScreen.picture(stand_rope) && $gameScreen.picture(stand_rope)._name == FileName){
      }else{
        if(RopePicFileNum != 0){//取得した装備タグの衣装ファイル名が0(全裸)以外の場合      
          $gameScreen.showPicture(stand_rope,FileName,0,Stand1X,Stand1Y,scale,scale,255,0)
        }else{
          $gameScreen.erasePicture(stand_rope)//全裸の場合消去
        }
      }
      //脚　変身中は反映なし
      ...
      if($gameScreen.picture(stand_leg) && $gameScreen.picture(stand_leg)._name == FileName){
        ...
        if(LegPicFileNum != 0 && StandAltFlag == 0 || LegPicFileNum != 0 && Cosplay == 1){//取得した装備タグの衣装ファイル名が0(全裸)以外、かつ未変身        
		  ...
		  var P = new Game_Picture();
		  P.show(FileName,0,Stand1X,Stand1Y,scale,scale,LegOpacity,0);
      ...
      if($gameScreen.picture(stand_under) && $gameScreen.picture(stand_under)._name == FileName){
        ...
        if(UnderPicFileNum != 0){//取得した装備タグの衣装ファイル名が0(全裸)以外の場合  
		  ...
		  var P = new Game_Picture();
		  P.show(FileName,0,Stand1X,Stand1Y,scale,scale,240,0);
	  ...
	  if(MouthPicFileNum == 0){$gameScreen.erasePicture(stand_mouth);}
		  ...
		  FileName = 'mouth/' + BasePoseFileName + "_mouth_" + MouthPicFileNum;
	  ...
	  if(StandPoseID == 4){//4(拘束)
		...
		else if(VaginaStateID == 1){Dif1PicFileName = "man01_penis_v";Dif2PicFileName = "man01";}
		...
		else if(AnusStateID == 1 && MouthStateID == 1 && VaginaStateID == 1){Dif1PicFileName = "penis_maa";;Dif2PicFileName = "man01";}      
		else if(VaginaStateID == 1 && MouthStateID == 1){Dif1PicFileName = "penis_m";;Dif2PicFileName = "man01";}
	  ...
	  if(weaponIndex) Dif1PicFileName = "weapon" + weaponIndex;

	  if(VaginaStateID > 0) $gameActors.actor(1).biriSocks();
	

      FileName = BasePoseFileName + "_sexual_" + Dif1PicFileName;
	  if(weaponIndex) FileName = 'weapon/' + FileName;
      ...
      if (StandAnimeX != 0 || StandAnimeY != 0){
		...
		MovePic(stand_rope,Stand1X,Stand1Y,StandMoveX,StandMoveY,StandAnimeWait)
        ...
        MovePic(stand_mark1,Stand1X,Stand1Y,StandMoveX,StandMoveY,StandAnimeWait)
		MovePic(stand_mark2,Stand1X,Stand1Y,StandMoveX,StandMoveY,StandAnimeWait)
...
if (command === 'ResetStandEro') {//消去1
  ...
  $gameScreen.erasePicture(100) //这俩为敌人图层
  $gameScreen.erasePicture(70) //呼吸
  $gameScreen.erasePicture(71)
}
    ...
    if (command === 'StandAnimation') {//アニメーションのみ
      ...
      var stand_semenmouth = stand_bigin + 25; //尾巴处理到此为止
	  var effect_breath = stand_bigin + 36; //229
      var stand_diffront = stand_bigin + 37;
