      ...
      if (command === 'CommonEXP') {
		...
		if(Orgasm > 0){
			...
			if($gameParty.inBattle()){
				$gameActors.actor(1).gainHp(-Math.round($gameActors.actor(1).mhp * Math.max(Orgasm * 10 - $gameActors.actor(1)._tp,0) / 250));
			}else{
				$gameActors.actor(1).gainHp(-Math.round(Math.min($gameActors.actor(1).mhp * Math.max(Orgasm * 10 - $gameActors.actor(1)._tp,0) / 250,$gameActors.actor(1).hp - 1)));
			}
			var a = $gameActors.actor(1).mmp * Orgasm / 30;
			if($gameActors.actor(1).mp > -$gameActors.actor(1).mmp){//触手吸收
				$gameSwitches._data[2916] = true;
				$gameVariables._data[4893] = Math.min($gameActors.actor(1).mp,a) * 4;
			}else{
				$gameSwitches._data[2916] = false;
			}
			$gameActors.actor(1).gainMp(-a);
			$gameActors.actor(1).gainSilentTp(-Math.round(10 * Orgasm));
			//事件附加余韵
