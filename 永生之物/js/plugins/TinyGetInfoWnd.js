 ...
 * @default 0
 *
 * @param Y position offset
 * @desc ウィンドウの表示位置のオフセットです。
 * @default 0
 * @type number
 *
 * @param Display Loss
...
(function() {
  ...
  var yPosOffset = Number(parameters['Y position offset'] || 0);
  var ignoreEscape = Boolean(parameters['Ignore Escape Characters']);
  ...
  Window_GetInfo.prototype.initialize = function(id, type, text, value) {
    Window_Base.prototype.initialize.call(this, -24, 0, Graphics.width + 48, 105);
    this.reset();
  ...
  Window_GetInfo.prototype.setPosition = function(value) {
    ...
    // set Y position
    if(yPosType === 0){
      this.y = this.index * 60 + yPosOffset;
    } else {
      this.y = 520 - (this.index * 60) - yPosOffset;
    }
  };
  ...
  Window_GetInfo.prototype.drawContents = function(id, type, text, value) {
    ...
    this.contents.fillRect(0, 21, Graphics.width + 24, 36, '#000000');
  ...
  Window_GetInfo.prototype.drawText = function(text, x, y, maxWidth, align){
    if(ignoreEscape)text = this.escapeCharacters(text);
    this.contents.drawText(text, x, y, maxWidth, this.lineHeight(), align);
  };

  Window_GetInfo.prototype.escapeCharacters = function(text){
    text = text.replace(/\\/g, '\x1b');
    text = text.replace(/\x1b\x1b/g, '\\');
    text = text.replace(/\x1b\w+\[(\d+)\]/gi, "");
