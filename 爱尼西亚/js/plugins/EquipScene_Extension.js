...
Scene_Equip.prototype.createCommandWindow = function() {
    ...
    this._commandWindow.setHelpWindow(this._helpWindow);//改変
...
Scene_Equip.prototype.callstand = function() {//立ち絵表示。
    var obj = this
    callmenustand.call(obj)
  };
...
Scene_Equip.prototype.slotWindowRect = function() {
    ...
    const wx = this.statusWidth();//パラメータ表示の幅
    const wy = commandWindowRect.y + commandWindowRect.height;//
    const ww = Graphics.boxWidth - standarea() - this.statusWidth();//立ち絵領域の確保
    let wh = this.mainAreaHeight() - commandWindowRect.height;
    //console.log(this.mainAreaHeight())
    if (LayoutMode >= 1) wh = 384;//244
...
Scene_Equip.prototype.itemWindowRect = function() {
    ...
    if (LayoutMode === 1) {
        ...
        const ww = slotWindowRect.width - standarea();
