 ...
 * @param Type
 * @text パーツタイプ
 * @desc ファイル名生成に使用する文字列です(例:Socks)
 * @default
 * 
 * @param Opacity
 * @text 不透明度
...
(function () {
    ...
    class StandPictureParam {
        ...
        updatePictureFiles() {
            this._standPictures.forEach(picture => {
                ...
                if (picture.UseFunction) {
                    picture.FileName = getCharacterPictureFileName(this._actor, picture);
                } else {
    ...
    window.StandPictureParam = StandPictureParam;
    ...
    class Sprite_StandPicture extends Sprite {
        ...
        update() {
            ...
            for (let i = 0; i < this.children.length; i++) {
                isReady = isReady && this.children[i].isReady();
            }
            if (isReady) {
                this.updatePosition();
                this.updateLayers();
    ...
    window.Sprite_StandPicture = Sprite_StandPicture;
    ...
    class Sprite_StandPictureChild extends Sprite_Clickable {
        ...
        update() {
            ...
            if (this._picture.Blend) this._blendMode = PIXI.BLEND_MODES[this._picture.Blend];
        }
        ...
        isReady() {
            return !this.loadingBitmap || this.loadingBitmap.isReady();
        }
        ...
        updateVisibility() {
            var opacity = this._picture.Opacity;
            if (typeof opacity === "string" || opacity instanceof String) opacity = ConvertCC(opacity)//改変、文字列の場合変数に変換して取得
